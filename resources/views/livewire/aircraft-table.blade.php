<div class="w-full overflow-hidden">
    <section class="w-full">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-500">
                            <tr
                                ...
                                <th scope="col" class="px-4 py-3">اسم الطائرة </th>
                                <th scope="col" class="px-4 py-3">طراز الطائرة</th>
                                <th scope="col" class="px-4 py-3"> الصانع </th>
                                <th scope="col" class="px-4 py-3">الحالة </th>
                                <th scope="col" class="px-4 py-3"> رقم التسجيل </th>
                            ...
                            @forelse ($aircrafts as $aircraft)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    <th scope="row" class="px-4 py-3">
                                        <p class="font-semibold">{{ $aircraft->aircraft_name }}</p>
                                    </th>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $aircraft->aircraft_code }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $aircraft->manufacturer }}</td>
                                    <td class="px-4 py-3 text-xs">
                                        <span
                                            class="px-4 py-1 font-semibold leading-tight rounded-full {{ $aircraft->status == 'active' ? 'text-green-700 bg-green-100 dark:bg-green-700 dark:text-green-100' : 'text-red-700 bg-red-100 dark:bg-red-700 dark:text-red-100' }}">
                                            {{ $aircraft->status }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $aircraft->registration_number }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        {{ $aircraft->user->name }}
                                    </td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        @if ($aircraft->created_at)
                                            {{ $aircraft->created_at->format('Y-m-d') }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    ...
                                    <td class="flex justify-center px-4 py-3">
                                        <div class="w-fit flex items-center text-sm">
                                            ...
                                            <button
                                                onclick="confirmDelete({{ $aircraft->id }}, '{{ $aircraft->aircraft_name }}')"
                                                id="delete-button_{{ $aircraft->id }}"
        ...
        {{-- delete row --}}
        <script>
            function confirmDelete(id, aircraftName) {
                window.dispatchEvent(new CustomEvent('swalConfirm', {
                    detail: {
                        title: 'هل انت متاكد ؟',
                        html: 'انت تريد حذف <strong>' + aircraftName + '</strong>',
                        id: id
                    }
                }));
            }
