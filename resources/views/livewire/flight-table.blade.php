<div class="w-full overflow-hidden">
    <section class="w-full">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-500">
                            <tr
                                ...
                                <th scope="col" class="px-4 py-3">رقم الرحلة </th>
                                ...
                                <th scope="col" class="px-4 py-3"> نوع الرحلة </th>
                                <th scope="col" class="px-4 py-3"> عدد ساعات الرحلة </th>
                                <th scope="col" class="px-4 py-3"> رقم تسجيل الطائرة  </th>
                            ...
                            @forelse ($flights as $flight)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    <th scope="row" class="px-4 py-3">
                                        <p class="font-semibold">{{ $flight->aircraft->aircraft_name }}</p>
                                    </th>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $flight->flight_number }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $flight->flight_date }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $flight->originAirport->airport_name }}
                                    </td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        {{ $flight->destinationAirport->airport_name }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        {{ $flight->flight_type }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">
        ...
        {{-- delete row  --}}
        <script>
            function confirmDelete(id, flightNum) {
                window.dispatchEvent(new CustomEvent('swalConfirm', {
                    detail: {
                        title: 'هل انت متاكد ؟',
                        html: 'انت تريد حذف الرحلة رقم <strong>' + flightNum + '</strong>',
                        id: id
                    }
                }));
            }
