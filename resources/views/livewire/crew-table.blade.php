<div>
    <x-employee.data-table :paginator="$crews" per-page-id="crew-per-page" class="responsive-card-table">
        <x-slot:head>
            <tr>
        ...
        @forelse ($crews as $crew)
            <tr wire:key="crew-{{ $crew->id }}">
                <td data-label="الاسم الأول" class="font-medium">{{ $crew->first_name }}</td>
                <td data-label="اسم الأب">{{ $crew->last_name }}</td>
                <td data-label="اللقب">{{ $crew->nickname ?: 'N/A' }}</td>
                <td data-label="الرقم المالي">{{ $crew->financial_number }}</td>
                <td data-label="الوظيفة">{{ $crew->job->job_name }}</td>
                <td data-label="حالة فرد الطاقم">
