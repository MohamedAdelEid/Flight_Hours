<div>
    ...
    <section class="w-full">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-500">
                            <tr
                                ...
                                <th scope="col" class="px-4 py-3">اسم المطار</th>
                                <th scope="col" class="px-4 py-3"> كود المطار </th>
                            ...
                            @forelse ($airports as $airport)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    <th scope="row" class="px-4 py-3">
                                        <p class="font-semibold">{{ $airport->airport_name }}</p>
                                    </th>
                                    <td class="px-4 py-3 text-sm font-bold">{{ $airport->airport_code }}</td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        {{ $airport->user->name }}
                                    </td>
                                    <td class="px-4 py-3 text-sm font-bold">
                                        @if ($airport->created_at)
                                            {{ $airport->created_at->format('Y-m-d') }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    ...
                                    <td class="flex justify-center px-4 py-3">
                                        <div class="w-fit flex items-center text-sm">
                                            ...
                                            <button
                                                onclick="confirmDelete({{ $airport->id }}, '{{ $airport->airport_name }}')"
                                                id="delete-button_{{ $airport->id }}"
        ...
        {{-- delete row --}}
        <script>
            function confirmDelete(id, airportName) {
                window.dispatchEvent(new CustomEvent('swalConfirm', {
                    detail: {
                        title: 'هل انت متاكد ؟',
                        html: 'انت تريد حذف <strong>' + airportName + '</strong>',
                        id: id
                    }
                }));
            }
